...
- Users can only insert/update their own profile data (though `is_approved` is typically managed by admins). *Note: The current implementation allows users to insert their profile via trigger, and potentially update non-sensitive fields if allowed by policy.*
